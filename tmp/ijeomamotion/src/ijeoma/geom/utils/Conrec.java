 ...
 * 
 * @see http://local.wasp.uwa.edu.au/~pbourke/papers/conrec/ for full
...
public class Conrec {
	...
	// private ConrecRender render = null;
	 ...
	 * @param ilb
	 *            ,iub,jlb,jub - index bounds of data matrix
	 * 
	 ...
	 * 
	 */
	public void computeContours(double[][] d, int ilb, int iub, int jlb,
						...
						if (z[k] >= dmin && z[k] <= dmax) {
							...
							for (m = 1; m <= 4; m++) {
								...
								if (case_value != 0) {
									...
									contours.add(new Contour((float) x1,
											(float) y1, (float) z[k],
											(float) x2, (float) y2,
											(float) z[k]));
								}
