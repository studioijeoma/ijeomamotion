...
ijeoma.motion.tween.Timeline = Timeline;
...
Timeline.prototype.insert = function(child, time) {
	...
	if(child.isKeyFrame()) {
		...
		if(child.getName() != null) this._keyFrameMap[child.getName()] = child;
	}
...
Timeline.prototype.addAll = function(children, time) {
	...
	if(keyFrame == null) {
		...
		this.insert(keyFrame, time);
	} else for(var j = 0; j < children.length; j++)
	keyFrame.add(children[j]);
...
Timeline.prototype.getCurrentKeyFrames = function() {
	...
	for(var i = 0; i < this._children.length; i++)
	if(this._children[i].isInsidePlayingTime(this.getTime())) currentKeyFrames.push(children[i]);
...
Timeline.prototype.getCurrentKeyFrameIndices = function() {
	...
	for(var i = 0; i < this._children.length; i++)
	if(this._children[i].isInsidePlayingTime(getTime())) indices.push(i);
