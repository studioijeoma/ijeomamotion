package ijeoma.motion.test;

import processing.core.PApplet;
import processing.core.PFont;
import ijeoma.motion.*;
import ijeoma.motion.tween.*;

public class Motion_Mouse_1 extends PApplet {

	int RED = color(255, 0, 0);
	int GREEN = color(0, 255, 0);

	PFont font;

	Tween t;

	public float x, y;

	public void setup() {
		size(400, 400);
		smooth();

		x = width/2;
		y = height/2;
		
		Motion.setup(this);
		
		t = new Tween();
		t.addProperty(new Property(this, "x", 0));
		t.addProperty(new Property(this, "y", 0));
		t.setDuration(50);
	}

	public void draw() {
		background(255);

		noStroke();
		fill(0);
		ellipse(x, y, 50, 50);

		int seekColor = lerpColor(GREEN, RED, t.getSeekPosition());
		
		stroke(seekColor);
		line(t.getSeekPosition() * width, 0, t.getSeekPosition() * width, height);

		String timeAsString = t.getTime() + " / " + t.getDuration();

		noStroke();
		fill(seekColor);
		text(timeAsString, width - textWidth(timeAsString) - 10, height - 10);
	}

	public void mousePressed() {
		t.getProperty("x").setEnd(mouseX);
		t.getProperty("y").setEnd(mouseY);
		t.play();
	}
}
